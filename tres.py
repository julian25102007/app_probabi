...
with st.sidebar:
    ...
    fuente = st.radio("Origen de datos", ["Datos sintéticos", "Cargar CSV"])
    ...
    if fuente == "Cargar CSV":
        ...
        if archivo is not None:
            try:
                ...
                st.success(f"✅ CSV cargado: {df_csv.shape[0]} filas, {df_csv.shape[1]} columnas")
                cols_num = df_csv.select_dtypes(include=np.number).columns.tolist()
                if cols_num:
                    columna = st.selectbox("Columna numérica a analizar", cols_num)
                    datos = df_csv[columna].dropna().values
                    n = len(datos)
                    st.info(f"n={n} valores válidos en **{columna}**")
                else:
...
if fuente == "Cargar CSV":
    if datos is None or len(datos) == 0:
        st.warning("⚠️ Carga un CSV y selecciona una columna numérica para continuar.")
        datos_ok = False
else:
